...
public class EnemySelectMove {
    ...
    public static Move enemySelectMove(PokeInfo enemyPokemon, PokeInfo myPokemon, Field field) {
        // 持つ技の個数
        int moveSize = enemyPokemon.haveMove().size();
        // ランダムな数値1~20
    ...
    private static List<Move> filterDisableRecovery(PokeInfo enemyPokemon) {
        // 残HP
        int remainingHP = enemyPokemon.currentHP().val();
        // 最大HP
        int maxHP = enemyPokemon.realHP();
        // 残HPの%
    ...
    private static List<Move> filterDisableAilment(PokeInfo enemyPokemon, PokeInfo myPokemon) {
        // 敵の状態異常を取得
        boolean isAilment = myPokemon.ailment().isSick();
        List<Move> filteredMoves;
        if (isAilment) {
    ...
    private static List<Move> filterDisableWeather(PokeInfo enemyPokemon, Field field) {
        ...
        switch (weather) {
            case DROUGHT ->
                    filteredMoves = enemyPokemon.haveMove().stream().filter(m -> m.baseMPrm() != BaseMvPrm.SUNNY_DAY).toList();
            case RAIN ->
                    filteredMoves = enemyPokemon.haveMove().stream().filter(m -> m.baseMPrm() != BaseMvPrm.RAIN_DANCE).toList();
            case SANDSTORM ->
                    filteredMoves = enemyPokemon.haveMove().stream().filter(m -> m.baseMPrm() != BaseMvPrm.SAND_STORM).toList();
            case HAIL ->
                    filteredMoves = enemyPokemon.haveMove().stream().filter(m -> m.baseMPrm() != BaseMvPrm.HAIL).toList();
            default -> filteredMoves = enemyPokemon.haveMove();
        }
        if (filteredMoves.size() == 0) {
            // フィルターした結果技候補がなくなる場合はすべての技を候補に入れる
            return enemyPokemon.haveMove();
    ...
    private static int maxDamage(PokeInfo enemyPokemon, PokeInfo myPokemon, Field field, int moveSize) {
        ...
        for (Move move : enemyPokemon.haveMove()) {
    ...
    private static int highPriorityCanDefeat(PokeInfo enemyPokemon, PokeInfo myPokemon, Field field, int moveSize) {
        ...
        // 防御側の残りHP
        int remainingHP = myPokemon.currentHP().val();
    ...
    private static int calcDamageForCPU(PokeInfo attackPoke, PokeInfo defencePoke, Field field, Move move) {
        ...
        if (moveSpecies == MoveSpecies.PHYSICAL) {
            attackVal = attackPoke.realAttack();
